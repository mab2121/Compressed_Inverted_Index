...
public class IndexerWork {
	...
	/**
	 * Should read the input corpus certain MB's a time build a temp index, sort it
	 * and save it to disk repeat till the entire index is read
	 */
	...
	/**
	 * @param buffer Reads the next chunk(e.g of 10Mb), creates a in memory inverted
	 *               index from it and persists it to disk
	 */
	...
	private void compressCompleteIndex() throws IOException {
		...
		IndexCompressor.compressIndexFile(toCompress, lexicon);
	}
	...
	public HashMap<Integer, String> pageTable = new HashMap<Integer, String>();

	// term -> begin offset in the disk based file
	// and the number of docs containing the word
	public static HashMap<String, List<Long>> lexicon = new HashMap<String, List<Long>>();
